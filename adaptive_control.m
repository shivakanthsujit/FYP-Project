function result= adaptive_control(y_true, y_ref, tolerance, dy, error_h)
global w_hat_rules
global parametersE
global parametersdy
    error = 0.5*(y_true - y_ref)^2;
    learning_rate = 0.005;
    error_f = fuzzify(error_h, 0);
    dy_f = fuzzify(dy, 1);
    ...
    if error >= tolerance
        ...
        if error_f(w_hat_rules{index}(1)) > dy_f(w_hat_rules{index}(2))
            ...
            new_epsilon = final_wt_update(old_epsilon, old_epsilon, error, parametersE{DF_index}(3), learning_rate, dy_f(w_hat_rules{index}(1)), parametersE{DF_index}(4), w(index), w_hats(index), k1,k2 );
            ...
            new_epsilon = final_wt_update(old_epsilon, old_epsilon, error, parametersdy{DF_index}(3), learning_rate, dy_f(w_hat_rules{index}(2)), parametersdy{DF_index}(4), w(index), w_hats(index), k1,k2);
