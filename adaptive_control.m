...
global parametersdy
    ...
    %end
        ...
        if error_f(w_hat_rules{index}(1)) > dy_f(w_hat_rules{index}(2))
            ...
            new_epsilon = final_wt_update(old_epsilon, old_epsilon, error, parametersE{DF_index}(3), 0.01, dy_f(w_hat_rules{index}(1)), parametersE{DF_index}(4), w(index), w_hats(index), k2,k2 );
            parametersE{DF_index}(2) = new_epsilon;
        else
            DF_index = w_hat_rules{index}(2);
            old_epsilon = parametersdy{DF_index}(2);
            new_epsilon = final_wt_update(old_epsilon, old_epsilon, error, parametersdy{DF_index}(3), 0.01, dy_f(w_hat_rules{index}(2)), parametersdy{DF_index}(4), w(index), w_hats(index), k2,k2);
